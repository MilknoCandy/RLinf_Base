...
RLT + STM-FIFO is a memory-augmented variant of RLT's Stage-2
actor-critic fine-tuning. RLT already compresses multimodal observations into a
compact RL token :math:`z_t` and learns a small actor-critic head on top of it.
STM-FIFO adds a per-environment short-term memory over the most recent RL-token
experiences, so the actor and critic can condition on the immediate trajectory
context instead of only on the current observation.
...
where :math:`z_t` is the RL token, :math:`a_t` is the executed action chunk,
:math:`r_t` is the chunk reward, and :math:`z_{t+1}` is the next RL token. The
short-term memory is a FIFO of the most recent :math:`K` experiences:

.. math::

   S_t = \{e_{t-K+1}, \dots, e_t\}.

At time :math:`t`, the memory reader produces a fixed-size feature :math:`m_t`.
Phase 1 uses a deterministic, uniform reader:

.. math::

   m_t = [\mathrm{mean}(z_i), \mathrm{mean}(r_i), r_{\mathrm{last}}],

where the means are taken over the experiences currently in :math:`S_{t-1}`.
The memory vector is concatenated with the current RL token, preserving RLT's
core design that the RL token remains the decision representation:
...
- actor input: :math:`[\mathrm{ref\_chunk}, z_t, m_t, \mathrm{proprio}]`
- critic input: :math:`[z_t, m_t, \mathrm{proprio}]`

3. Causality and Advantage
--------------------------

The memory update runs in the rollout worker so that :math:`z_{t+1}` is
available before the policy predicts :math:`a_t`:
...
Advantage :math:`A_t` is not available causally at rollout time, so Phase 1
uses reward as the reward-conditioned signal. Advantage-conditioned memory is
left for a later RL-trained memory phase.
...
- ``rlinf/algorithms/rlt/stm.py``
  Implements :class:`RLTSTMFIFO`, a per-environment FIFO with
  ``complete_and_retrieve`` and ``remember_current``.
...
- Memory reader is fixed and deterministic (mean pooling over RL tokens and
  reward scalars).
- Memory is reset on episode termination and at the first observation of a
  new rollout, when the environment worker sends a reset observation without
  reward/done context.
