...
"""Official-OpenPI-backed RLT Idea3 (text-only RL token)."""
...
class OpenPiIdea3ActionModel(OpenPiIdea1ActionModel):
    """RL token attends only to text-token keys."""
    ...
    def _apply_rl_token_attention(
        ...
        new_mask[:, rl_query_idx, :] = (
            att_2d_masks[:, rl_query_idx, :] & key_is_text.unsqueeze(0)
        )
        return new_mask
