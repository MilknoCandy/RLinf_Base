...
"""Factory for the feature-selectable RLT probe built on the official OpenPI path.

Mirrors :mod:`rlinf.models.embodiment.openpi` so checkpoint loading, data
config, normalization stats, and transforms are identical to ``openpi``. The
only difference is the concrete wrapper class and the extra
``rlt_feature_source`` knob.
"""
...
def get_model(cfg: DictConfig, torch_dtype=None):
    ...
    from rlinf.models.embodiment.openpi_rlt_probe.openpi_probe_model import (
        OpenPiProbeForRLActionPrediction,
    )
    ...
    rlt_feature_source = str(getattr(cfg.openpi, "rlt_feature_source", "all"))
    rlt_mode = str(getattr(cfg.openpi, "rlt_mode", "token"))
    ...
    if override_model_config_kwargs is not None:
        for key, val in override_model_config_kwargs.items():
            if key in ("rlt_feature_source", "rlt_mode"):
                continue
