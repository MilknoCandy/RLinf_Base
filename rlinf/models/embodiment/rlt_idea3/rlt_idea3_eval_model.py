...
"""Eval / Stage2 feature extraction model for the RLT Idea3 variant.

Mirrors :class:`RltIdea2EvalModel`, except the injected RL token is masked so
that it attends only to text-token keys in the VLM prefix pass.
"""
...
class RltIdea3EvalModel(RltIdea3ActionModel, OpenPiPytorchEvalActionModel):
    """Eval wrapper with the Idea3 text-only RL token feature extractor."""
    ...
    def extract_rlt_obs(self, env_obs: dict[str, Any]) -> dict[str, torch.Tensor]:
        ...
        prefix_attn_mask = self._apply_text_only_mask(
