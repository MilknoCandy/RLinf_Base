...
"""SFT action model for the RLT Idea3 variant.

Same as Idea1, except the injected RL token only attends to text tokens.
"""
...
from rlinf.models.embodiment.rlt_idea3.rlt_idea3_action_model import (
    RltIdea3ActionModel,
)


class RltIdea3SFTModel(RltIdea3ActionModel, RltIdea1SFTModel):
    """SFT wrapper with the Idea3 text-only RL token objective."""
    ...
    def _sft_forward_with_rlt_prefix(
        ...
        """Run one VLA pass and extract z_rl from the text-only RL token."""
        ...
        attn_mask = self._apply_text_only_mask(
        ...
        decoder_target, decoder_mask = self._select_rlt_text_prefix_embeddings(
            decoder_target, decoder_mask, num_image_tokens
        )
