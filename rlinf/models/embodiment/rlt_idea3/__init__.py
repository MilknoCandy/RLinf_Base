...
"""Factory for RLT Idea3 models (text-only learnable-token-in-VLM).

Usage in YAML::

    model:
      model_type: rlt_idea3
...
def _build_eval_model(
    ...
    *,
    ...
    from rlinf.models.embodiment.rlt_idea3.rlt_idea3_eval_model import (
        RltIdea3EvalModel,
    )
    ...
    if not config_name:
        raise ValueError(
            "actor.model.rlt_idea3.config_name is required for task='eval'."
        )
...
def _build_sft_model(
    ...
    *,
    ...
    from rlinf.models.embodiment.rlt_idea3.rlt_idea3_sft_model import (
        RltIdea3SFTModel,
    )

    return RltIdea3SFTModel(
        model,
        num_steps=num_steps,
        action_env_dim=action_env_dim,
        rlt_cfg=build_rlt_idea1_config(model_cfg),
...
def get_model(cfg: Any, torch_dtype: Any = None) -> Any:
    """Build a Pi0/Pi0.5 model with the RLT Idea3 objective."""
    import pathlib
    ...
    if safetensors_path is None and full_weights_path is None:
        raise FileNotFoundError(
            f"rlt_idea3 checkpoint not found at {model_path}."
        )
    ...
    if task is None:
        raise ValueError(
            "actor.model.rlt_idea3.task is required: set to 'sft' or 'eval'."
        )
    task = str(task).lower()
    ...
    if task == "eval":
        ...
        raise ValueError(
            f"actor.model.rlt_idea3.task={task!r} not supported; "
            "use 'sft' or 'eval'."
        )
    ...
    logger.info(
        "rlt_idea3[%s]: loaded %s (%.2fB params) from %s",
        task,
        pi0_config,
        n_params / 1e9,
        source,
