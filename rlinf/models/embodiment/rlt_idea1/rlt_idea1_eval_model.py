...
"""Eval / Stage2 feature extraction model for the RLT Idea1 variant.

Inherits the openpi eval pipeline (transforms, Euler sampler) from
``OpenPiPytorchEvalActionModel`` and overrides ``extract_rlt_obs`` to read
z_rl from the learnable token's VLM output position instead of a post-hoc
encoder.
"""
...
class RltIdea1EvalModel(RltIdea1ActionModel, OpenPiPytorchEvalActionModel):
    """Eval wrapper with the Idea1 learnable-token-in-VLM feature extractor."""
    ...
    def __init__(
        ...
        *,
        ...
        # Run the Idea1 base init (decoder-only module). We deliberately do
        # not call OpenPiPytorchEvalActionModel.__init__ so that its
        # RLTokenTransformer is never created; the eval attributes are set
        # below.
        RltIdea1ActionModel.__init__(
            self,
            pi0_model,
            num_steps=num_steps,
    ...
    # Eval pipeline methods (setup_wrappers, _repack_env_obs, input/output
    # transforms, _sample_actions_from_prefix_cache, predict_action_batch,
    # etc.) are inherited from OpenPiPytorchEvalActionModel via MRO.
    ...
    def extract_rlt_obs(self, env_obs: dict[str, Any]) -> dict[str, torch.Tensor]:
        ...
        Idea1: inject the learnable token, run the VLM prefix pass, and read
        z_rl from the token's output position.
        """
        self._require_rlt()
        ...
        # Build prefix + inject the learnable token, then run one LLM pass.
        ...
        # z_rl = hidden state at the learnable token position (last prefix
        # position). Project to the decoder token width for Stage2.
