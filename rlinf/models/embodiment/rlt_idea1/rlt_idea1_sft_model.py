...
"""SFT action model for the RLT Idea1 variant.

During Stage1 SFT the learnable token is injected into the VLM prefix. The
VLM attention layers compress the observation into the token's output
position (z_rl), which is then decoded to reconstruct the prefix. The total
loss is ``rlt_loss + rlt_alpha * vla_loss``.
"""
...
class RltIdea1SFTModel(RltIdea1ActionModel):
    """SFT wrapper with the Idea1 learnable-token-in-VLM objective."""
    ...
    def _sft_forward_with_rlt_prefix(
        ...
        """Run one flow-matching VLA pass and extract z_rl from the VLM token.

        Returns:
            ``(loss, decoder_target, decoder_mask, z_rl)`` where
            ``loss`` is the ``[B, action_horizon]`` per-timestep VLA loss,
            ``decoder_target`` / ``decoder_mask`` are the prefix hidden states
            (without the learnable token) used as the reconstruction target,
            and ``z_rl`` is the VLM hidden state at the token position.
        """
        ...
        # Idea1: inject the learnable token into the prefix.
