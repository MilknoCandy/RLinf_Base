...
"""Factory for RLT Idea1 models (learnable-token-in-VLM).

Usage in YAML::

    model:
      model_type: rlt_idea1
...
def _build_eval_model(
    ...
    *,
    ...
    from rlinf.models.embodiment.rlt_idea1.rlt_idea1_eval_model import (
        RltIdea1EvalModel,
    )
    ...
    if not config_name:
        raise ValueError(
            "actor.model.rlt_idea1.config_name is required for task='eval'."
        )
...
def get_model(cfg: Any, torch_dtype: Any = None) -> Any:
    """Build a Pi0/Pi0.5 model with the RLT Idea1 objective."""
    import pathlib
    ...
    if safetensors_path is None and full_weights_path is None:
        raise FileNotFoundError(
            f"rlt_idea1 checkpoint not found at {model_path}."
        )
    ...
    if task is None:
        raise ValueError(
            "actor.model.rlt_idea1.task is required: set to 'sft' or 'eval'."
        )
    task = str(task).lower()
    ...
    if task == "eval":
        ...
        raise ValueError(
            f"actor.model.rlt_idea1.task={task!r} not supported; "
            "use 'sft' or 'eval'."
        )
    ...
    logger.info(
        "rlt_idea1[%s]: loaded %s (%.2fB params) from %s",
        task,
        pi0_config,
        n_params / 1e9,
        source,
