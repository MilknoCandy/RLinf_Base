...
"""RLT-VLM configuration: learnable-token-in-VLM variant of RLT.

Unlike the standard RLT (post_vlm encoder), this variant injects a learnable
token into the VLM prefix and lets the VLM's own attention layers compress the
observation. Only a lightweight decoder is trained for reconstruction.
"""
...
class RLTConfigVLM:
    """RLT-token config for the vlm_internal (learnable-token-in-VLM) variant.

    Unlike ``OpenPiPytorchRLTConfig`` which uses a post-hoc transformer encoder,
    this config uses a single learnable token injected into the VLM prefix.
    The VLM's attention layers do the compression; only a decoder is trained.
    """
...
def build_rlt_config_vlm(model_cfg: Any) -> RLTConfigVLM:
    """Build RLT-VLM config from ``actor.model.openpi_vlm`` config block."""
