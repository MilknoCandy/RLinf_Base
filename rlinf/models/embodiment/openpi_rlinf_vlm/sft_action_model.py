...
"""SFT action model for the VLM-internal RLT variant.

Extends the standard SFT action model to inject a learnable token into the
VLM prefix during training. The VLM's attention layers compress the
observation into the token's hidden state (z_rl), which is then decoded
...
class OpenPiPytorchSFTActionModelVLM(OpenPiPytorchActionModelVLM):
    """SFT variant with learnable-token-in-VLM RLT.

    With ``use_rlt=True``:
    1. A learnable token is injected into the VLM prefix.
    2. The VLM's attention layers compress the observation into this token's
       hidden state (z_rl).
    3. A lightweight decoder reconstructs the prefix from z_rl (MSE loss).
    4. Total loss = rlt_loss + rlt_alpha * vla_loss.
    """
    ...
    def sft_forward(self, data: Any) -> torch.Tensor:
        ...
        per_timestep_loss, prefix_output, prefix_mask, z_rl = (
            self._sft_forward_with_rlt_prefix(observation, actions)
        )
        vla_loss = per_timestep_loss.mean()
        rlt_loss, _ = self._rlt_forward(prefix_output, prefix_mask, z_rl=z_rl)
        return {
    ...
    def _observation_to_device(self, observation: Any) -> Observation:
        return OpenPiPytorchSFTActionModel._observation_to_device(self, observation)

    def _actions_to_device(self, actions: Any) -> torch.Tensor:
        return OpenPiPytorchSFTActionModel._actions_to_device(self, actions)

    # --- Core training method: inject learnable token, extract z_rl ---
    ...
    def _sft_forward_with_rlt_prefix(
        ...
        """Compute VLA loss + extract z_rl from VLM learnable token position.

        Returns:
            (loss, decoder_target, decoder_mask, z_rl) 4-tuple.
            - loss: [B, action_horizon] per-timestep VLA loss
            - decoder_target: [B, S'', D] prefix hidden states for decoder
            - decoder_mask: [B, S''] decoder target mask
            - z_rl: [B, embed_dim] compressed feature from VLM token position
        """
        ...
        # 1. Embed prefix normally (no learnable token yet)
        ...
        # 2. Inject learnable token into prefix (post-hoc concatenation)
        rl_token = self._get_vlm_rl_token(batch_size)
        prefix_tokens, prefix_mask, prefix_ar_mask = self._inject_rl_token_into_prefix(
            prefix_tokens, prefix_mask, prefix_ar_mask, rl_token
        )

        # 3. Run LLM on combined prefix + suffix
