...
RLT + STM-FIFO 是 RLT 第二阶段 actor-critic 微调的带记忆变体。RLT 已经将多模态观测压缩为紧凑的 RL token $z_t$，并在其上学习一个小型 actor-critic 头。STM-FIFO 进一步为每个环境维护一段最近的 RL-token 经验，使 actor 和 critic 不再只依赖当前观测，而能显式利用近期轨迹上下文。
...
其中 $z_t$ 是 RL token，$a_t$ 是已执行的动作块，$r_t$ 是块级奖励，$z_{t+1}$ 是下一状态的 RL token。短期记忆保存最近 $K$ 条经验的 FIFO：

$$S_t = \{e_{t-K+1}, \dots, e_t\}.$$

在 $t$ 时刻，记忆读取器输出固定维度特征 $m_t$。Phase 1 使用确定性的均匀读取器：

$$m_t = [\mathrm{mean}(z_i), \mathrm{mean}(r_i), r_{\mathrm{last}}],$$

均值在当前 $S_{t-1}$ 中的经验上计算。记忆向量与当前 RL token 拼接，从而保持 RLT 的核心设计，即 RL token 仍是决策中间表示：

$$z'_t = [z_t, m_t].$$

第二阶段策略随后使用：

- actor 输入：$[\mathrm{ref\_chunk}, z_t, m_t, \mathrm{proprio}]$
- critic 输入：$[z_t, m_t, \mathrm{proprio}]$

## 3. 因果性与优势

记忆更新在 rollout worker 中完成，确保策略预测 $a_t$ 时已经能够使用 $z_{t+1}$：
...
优势 $A_t$ 无法在 rollout 时因果地获得，因此 Phase 1 使用奖励作为 reward-conditioned 信号；优势条件化记忆留待后续 RL 训练记忆阶段。
...
- `rlinf/algorithms/rlt/stm.py`  
  实现 `RLTSTMFIFO`，提供每个环境的 FIFO，以及 `complete_and_retrieve` 和 `remember_current`。
