...
ENV_NAME=""
VENV_DIR=".venv"
PYTHON_VERSION="3.10.20"    # ori: 3.11.14
...
USE_MIRRORS=0
GITHUB_PREFIX=""
NO_ROOT=1   # non-root users should not install system dependencies by default; set --no-root to override
...
Common options:
    ...
    --conda-name <name>    Name of the conda environment (default: starvla_env_name).
...
parse_args() {
    ...
    while [ "$#" -gt 0 ]; do
        case "$1" in
            ...
            --conda-name)
                if [ -z "${2:-}" ]; then
                    echo "--conda-name requires a name argument." >&2
                    exit 1
                fi
                CONDA_NAME="${2:-}"
                shift 2
...
configure_nvidia() {
    ...
    # 1. INIT CUDA
    export CUDA_HOME="/path/to/cuda-12.8"  # Set this to the path where CUDA is installed
    export PATH="$CUDA_HOME/bin:$PATH"
    export LD_LIBRARY_PATH="$CUDA_HOME/lib64:$LD_LIBRARY_PATH"

    export CPATH="/path/to/cudnn/include:$CPATH"  # Set this to the path where cuDNN headers are located
    export LIBRARY_PATH="/path/to/cudnn/lib:$LIBRARY_PATH"  # Set this to the path where cuDNN libraries are located
    export LD_LIBRARY_PATH="/path/to/cudnn/lib:$LD_LIBRARY_PATH"  # Set this to the path where cuDNN libraries are located
    export CPATH="/path/to/nccl/include:$CPATH"  # Set this to the path where NCCL headers are located
    export LIBRARY_PATH="/path/to/nccl/lib:$LIBRARY_PATH"  # Set this to the path where NCCL libraries are located
    export LD_LIBRARY_PATH="/path/to/nccl/lib:$LD_LIBRARY_PATH"  # Set this to the path where NCCL libraries are located

    # 2. INIT CONDA (already initialized)

    # 3. INIT Rendering
    # change render backend from osmesa to egl (CPU->GPU)
    export RENDER_BACKEND="egl"     # button for rendering backend, options: osmesa, egl, glfw
    export MUJOCO_GL=egl            # MuJoCo Off-screen GPU Rendering with EGL
    export PYOPENGL_PLATFORM=egl      # PyOpenGL Off-screen GPU Rendering with EGL"
    # Set this to the path where NVIDIA driver is installed
    export NVGL_DIR="/path/to/NVIDIA-Linux-x86_64-53.183.01"  # Directory for NVIDIA GL/EGL
    export OSMESA_DEP_DIR="/path/to/libGL.so.1"  # Set this to the path where libGL.so.1 is located for OpenCV
    export __EGL_VENDOR_LIBRARY_DIRS=$NVGL_DIR   # GLVND find 10_nvidia.json -> select NVIDIA EGL driver
    export NVIDIA_DRIVER_CAPABILITIES=all   # Unlock graphics capabilities for NVIDIA driver within container
    # seq: Mesa libGL -> NVIDIA GLVND
    export LD_LIBRARY_PATH=${OSMESA_DEP_DIR}:${NVGL_DIR}:${LD_LIBRARY_PATH}  # Add NVIDIA driver directory to LD_LIBRARY_PATH
    #------------------------------------------ SETTINGS for NVIDIA GPU ------------------------------------------
    ...
    # read torch version from pyproject.toml
    ...
    if [ -n "$_torch_ver" ] && [[ "$_tmaj" =~ ^[0-9]+$ ]] && [[ "$_tmin" =~ ^[0-9]+$ ]] \
        ...
        if [ "$_cpu_only" -eq 1 ]; then
            ...
            echo "[install.sh] Routing torch ${_torch_ver} (cpu) through ${PLATFORM_TORCH_INDEX}."
            ...
            echo "[install.sh] Could not resolve a driver-compatible CUDA wheel for torch ${_torch_ver}; leaving torch on the default index." >&2
        fi
    fi
}
...
configure_platform() {
    ...
    echo "[install.sh] platform=${PLATFORM}"
}
...
create_and_sync_conda_venv() {
    # 1. extract major.minor from PYTHON_VERSION (e.g. 3.11.4 -> 3.11)
...
EOF
        ...
        if [ "$active_python_mm" != "$required_python_mm" ]; then
            echo "Conda Python version mismatch: required ${required_python_mm}.x (from PYTHON_VERSION=${PYTHON_VERSION}), found ${active_python_mm}.x. Recreating conda-env..." >&2
...
EOF
    ...
    if [ "$PLATFORM_FLASH_ATTN_PREBUILT" -ne 1 ]; then
        ...
        pip uninstall flash-attn || true
        FLASH_ATTENTION_FORCE_BUILD=TRUE pip install "flash-attn==${flash_ver}" --no-build-isolation
        return 0
    fi
    # Detect Python tags
...
EOF
    ...
    for prebuilt_ver in "${prebuilt_flash_versions[@]}"; do
        ...
        for host in "${flash_attn_release_hosts[@]}"; do
            ...
            if pip install "${base_url}/${wheel_name}"; then
                return 0
            fi
...
install_starvla_model() {
    ...
    if [ -f "$starvla_path/requirements.txt" ]; then
        pip install -r "$starvla_path/requirements.txt"
    fi

    # Enforce RLinf-compatible runtime pins to avoid known breakages.
    pip install -r "$SCRIPT_DIR/embodied/models/starvla.txt"
    pip install -e "$starvla_path" --no-deps
...
export MINICONDA_PATH="/path/to/miniconda3"  # Set this to the path where Miniconda is installed
